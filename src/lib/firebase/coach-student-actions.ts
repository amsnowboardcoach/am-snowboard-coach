...
export async function publishStudentPassportChanges(
  studentId: string,
  input: {
  ...
  const res = await fetch(
    `/api/coach/students/${encodeURIComponent(studentId)}/trick-progress`,
    {
      method: "POST",
      headers: {
...
/** @deprecated Usa publishStudentPassportChanges */
export async function publishStudentTrickUpdates(
  studentId: string,
  updates: TrickProgressPublishUpdate[],
): Promise<void> {
  await publishStudentPassportChanges(studentId, { trickUpdates: updates });
}
