...
import { coachDeleteStudent } from "@/lib/account/delete-account-client";
...
export function DeleteStudentButton({
  ...
  async function handleDelete() {
    const msg = `¿Eliminar la cuenta de ${studentName} (${studentEmail})?\n\nSe borrarán perfil, vídeos, Tribu, mercadillo y reservas. No se puede deshacer.`;
    if (!confirm(msg)) return;

    setLoading(true);
    setError(null);
    try {
      await coachDeleteStudent(studentId);
      onDeleted?.();
      if (redirectToList) {
