...
interface CoachAlumnoContentPanelProps {
  alumnoId: string;
  alumnoName: string;
}
...
export function CoachAlumnoContentPanel({
  alumnoId,
  alumnoName,
}: CoachAlumnoContentPanelProps) {
  ...
  const loadTribe = useCallback(async () => {
    ...
    setError(null);
    try {
      setTribePosts(await fetchTribePostsByAuthor(alumnoId));
    } catch (err) {
    ...
    <div className="space-y-10">
      ...
      <section>
        ...
        <p className="mt-1 text-sm text-zinc-500">
          Material que {alumnoName} ha subido para revisión.
        </p>
        <div className="mt-4">
          <CoachVideoReviewPanel alumnoId={alumnoId} />
        </div>
      </section>
