...
import { publishStudentPassportChanges } from "@/lib/firebase/coach-student-actions";
import {
...
export function StudentTrickManager({ student }: StudentTrickManagerProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [data, sectionNotes] = await Promise.all([
        mergeTricksWithProgress(student.uid),
        fetchPassportSectionNotes(student.uid),
      ]);
      setTricks(data);
  ...
  async function confirmAndNotify() {
    ...
    setError(null);
    try {
      await publishStudentPassportChanges(student.uid, {
