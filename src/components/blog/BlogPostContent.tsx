    ...
    <div className="blog-markdown">
      <ReactMarkdown
        components={{
          ...
          ul: ({ children }) => (
            <ul className="mt-4 list-disc space-y-2 pl-6 text-lg text-zinc-400">{children}</ul>
          ),
          ol: ({ children }) => (
            <ol className="mt-4 list-decimal space-y-2 pl-6 text-lg text-zinc-400">{children}</ol>
          ),
          ...
          a: ({ href, children }) => {
            if (href?.startsWith("/")) {
              return (
                <Link href={href} className="link-accent underline-offset-2 hover:underline">
                  {children}
                </Link>
              );
            }
