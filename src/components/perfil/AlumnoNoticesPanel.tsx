...
interface AlumnoNoticesPanelProps {
  alumnoId: string;
  /** Vista compacta en perfil (solo los últimos). */
  compact?: boolean;
}

export function AlumnoNoticesPanel({
  alumnoId,
  compact = false,
}: AlumnoNoticesPanelProps) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const list = await fetchAlumnoCoachNotices(alumnoId, compact ? 3 : 50);
      setNotices(list);
    } catch (err) {
      setError(
  ...
  async function handleOpen(notice: CoachNoticeDoc) {
    ...
    try {
      await markCoachNoticeRead(alumnoId, notice.id);
      await load();
    } catch {
      /* ignore */
    }
  ...
  async function handleMarkAllRead() {
    try {
      await markAllCoachNoticesRead(alumnoId, notices);
      await load();
    } catch {
      /* ignore */
    }
